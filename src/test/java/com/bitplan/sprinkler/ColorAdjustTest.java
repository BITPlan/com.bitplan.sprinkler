...
public class ColorAdjustTest extends Application {
  ...
  Color[] presetColors = new Color[] { Color.RED, Color.GREEN, Color.BLUE,
      Color.YELLOW, Color.MAGENTA, Color.CYAN, Color.DODGERBLUE,
      Color.LIGHTGREY, Color.DARKGRAY, Color.BLACK, Color.WHITE, Color.BROWN };
  ...
  public ImageView getIcon(String name) {
    URL iconUrl = this.getClass().getResource("/icons/" + name + ".svg");
    if (iconUrl != null) {
      int size = 256;
      ImageView iconImage = new ImageView(iconUrl.toString());
      iconImage.setFitHeight(size);
      iconImage.setFitWidth(size);
      return iconImage;
    }
    return null;
  }
  ...
  public void start(Stage primaryStage) {
    ...
    ImageView modified;
    // create Icon
    String name = "watering_canx";
    SvgImageLoaderFactory.install();
    original = this.getIcon(name);
    // modified = this.getIcon(name);
    ...
    // presets: show colors as rectangles in the combobox list, as hex color in
  ...
  private void updateColor() {
    ...
    // create target color
    targetColor = Color.rgb((int) redSlider.getValue(),
    ...
    // System.out.println("Target color: " + targetColor + ", hue 0..360: " +
    // targetColor.getHue() + ", hue 0..1: " + hue);
  ...
  /**
   * Create an alpha masked ball with gradient colors from White to
   * Black/Transparent. Used e. g. for particles.
   *
   * @param radius
   * @return
   */
  public static Node createAlphaMaskedBall(double radius) {
    ...
    RadialGradient gradient1 = new RadialGradient(0, 0, 0, 0, radius, false,
        CycleMethod.NO_CYCLE, new Stop(0, Color.WHITE.deriveColor(1, 1, 1, 1)),
  ...
  public static double map(double value, double start, double stop,
      double targetStart, double targetStop) {
    return targetStart
        + (targetStop - targetStart) * ((value - start) / (stop - start));
  }
